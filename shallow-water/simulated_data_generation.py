...
if not os.path.isdir('uniform01_2_1'):
    os.makedirs('uniform01_2_1')
...
try:
    ...
    for ii in range(0,1):  ################原来是30000 刘雅慧
        ...
        ########################自己写的############################## 刘雅慧
            if i%100 ==0:
                ...
                plt.colorbar(im)
                t=SW.time
                plt.title('H: T=%1.3f'%t, fontsize=16)
                #plt.title("$h (t = 0.05)$")
                ...
                plt.colorbar(im)
                plt.title('u: T=%1.3f'%t, fontsize=16)
                ...
                plt.colorbar(im)
                plt.title('v: T=%1.3f'%t, fontsize=16)
                #plt.savefig("Figures/v_t005.eps", format = "eps")
                plt.show()
                plt.close()   
        
                im = plt.imshow(y_obs, interpolation = "None")
                plt.colorbar(im)
                plt.title('y_obs: T=%1.3f'%t, fontsize=16)
                #plt.savefig("Figures/v_t005.eps", format = "eps")
                plt.show()
                plt.close()  
